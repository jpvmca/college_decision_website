...
type Props = { item: CollegeDecisionResult; showCompare?: boolean };
...
export default function CollegeDecisionCard({ item, showCompare = true }: Props) {
  return <article className="decision-result college-decision-card">
    ...
    {(item.averageYearFee || item.averagePackage || item.highestPackage || item.nirfRank) && <div className="decision-card-highlights">{item.averageYearFee && <span>Average yearly fee <b>{money(item.averageYearFee)}</b></span>}{item.averagePackage && packageMoney(item.averagePackage) && <span>Average placement <b>{packageMoney(item.averagePackage)}</b></span>}{item.highestPackage && packageMoney(item.highestPackage) && <span>Highest placement <b>{packageMoney(item.highestPackage)}</b></span>}{item.nirfRank && <span>NIRF <b>Rank {item.nirfRank}{item.nirfOutOf ? ` / ${item.nirfOutOf}` : ''}</b></span>}</div>}
