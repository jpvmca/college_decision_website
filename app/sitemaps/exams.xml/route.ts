...
export async function GET() {
  ...
  try {
    const response = await fetch(`${backend}/exams/published-slugs`, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Backend returned ${response.status}`);
    const payload = await response.json() as { data?: Array<{ slug: string; updatedAt?: string | null }> };
    const body = `<?xml version="1.0" encoding="UTF-8"?>` +
      `<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">` +
      (payload.data || []).map((row) => `<url><loc>${xml(`${base}/exams/${row.slug}`)}</loc>${row.updatedAt ? `<lastmod>${new Date(row.updatedAt).toISOString()}</lastmod>` : ''}<changefreq>monthly</changefreq><priority>0.7</priority></url>`).join('') +
      `</urlset>`;
