...
export default async function ExamsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <main className="section"><div className="wrap">
    ...
    <div className="article-list">{result.data.length ? result.data.map((exam, index) => <article className="article-card" id={`exam-${index + 1}`} key={exam.id}><div className="article-card-top"><span className="pill">{exam.course_name || 'Entrance exam'}</span><span className="muted">{Number(exam.institute_count || 0)} colleges</span></div><h2>{exam.name}</h2><p>{exam.course_name ? `${exam.name} is connected with ${exam.course_name} programmes and admission routes.` : `Explore colleges and programmes connected with ${exam.name}.`}</p><div className="article-facts"><span>{Number(exam.programme_count || 0)} mapped programmes</span>{exam.review_count ? <span>{exam.review_count} reviews · {Number(exam.average_rating).toFixed(1)}/5</span> : <span>Reviews not listed</span>}</div></article>) : <div className="card"><h2>Exam list unavailable</h2><p>Please try again shortly.</p></div>}</div>
